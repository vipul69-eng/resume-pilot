...
function detectFileInputs() {
  ...
  const inputs = [
    ...
    ...document.querySelectorAll(
      '[aria-label*="file"], [aria-label*="upload"], [aria-label*="attach"]'
    ),
  ];
  ...
  const fileInputElements = inputs.filter((el) => {
    ...
    const parent = el.closest(
      '[role="button"], button, .file-upload, [class*="upload"]'
    );
    if (parent) {
    ...
    const acceptsPDF = fileInputElements.some((input) => {
      if (input.type !== "file") {
        ...
        const actualInput = input
          .closest('[role="button"], button, .file-upload, [class*="upload"]')
...
function getAllFileInputs() {
  ...
  document.querySelectorAll("iframe").forEach((iframe) => {
    try {
      const iframeInputs =
        iframe.contentDocument?.querySelectorAll('input[type="file"]');
...
function highlightInput(input) {
  ...
  if (
    ...
    targetElement = input.closest(
      'button, [role="button"], .file-upload, [class*="upload"], label, div'
    );
  }
